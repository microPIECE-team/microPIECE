...
The pipeline analizes the miRNA set (either from the *full* or *minimal workflow*) for miRNA isoforms. Outgoing from previous benchmarks [@amsel2017evaluation], we chose `miraligner` from the `seqbuster` package [@pantano2009seqbuster] as miRNA isoform detection tool. Finally, a target prediction with `miranda` [@betel2008microrna] on the previously transferred orthologous CLIP regions is performed. 
